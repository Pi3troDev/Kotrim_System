...
import { InventoryReport } from '../../interfaces/report.interfaces';

@Component({
  selector: 'app-inventory-report-page',
  imports: [CurrencyPipe, MatIconModule, MatProgressSpinnerModule, StatTile, ReportsTabs, DateRangeFilter],
...
export class InventoryReportPage implements OnInit {
  ...
  load(): void {
    ...
    this.reportsService.getInventoryReport(this.toIsoDate(this.from()), this.toIsoDate(this.to())).subscribe({
      next: (report) => {
